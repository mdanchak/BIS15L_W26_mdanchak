...
``` 
command + option+i = new code chunk
...
```{r}
x <- 42 #this means that x gets the value of 42
x=42
```
...
```{r}
my_experiment= control + treatment
my_experiment
```
...
```{r}
mean(2,8,2)
#Won't work because these numbers have not been put into a vector
```
...
```{r}
my_numeric <- as.integer(my_integer)
```
