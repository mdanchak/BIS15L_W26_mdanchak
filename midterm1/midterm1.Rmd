...
Link to repository:
\
#https://github.com/mdanchak/BIS15L_W26_mdanchak
\
\
\
...
#Cuba,Hispaniola, Puerto Rico, Jamacia
```
\
\
\
...
anolis %>%
  ...
  labs (title = "Number of Anole Species from Each island", x= "Name of Island", y= "Amount of Species")

#Sampling isn't really equal across islands
```
\
\
\
...
anolis %>%
  distinct(habitat)
#Trunk-ground, Twig, Grass-bush, Trunk-crown
```
\
\
\
...
anolis %>%
  ...
  labs (title = "Number of Anole Species from Each habitat", x= "Name of Habitat", y= "Amount of Species")

#Sampling isn't really equal among the Habitats
```
\
\
\
...
anolis %>%
  ...
  labs( title = "How the Range of Hindlimb length in mm compares among different habitats", x= "Hindlimb Length (mm)", y="Habitat" )

#The range of hindlimb length varies from habitat to habitat as they all live in different habitats causing them to have different sizes. 
...
```{r}
anolis %>%
  mutate(ratio_of_hindlimb_to_body =log10(hindlimb_length_mm/body_length_mm))%>%
  mutate(ratio_of_tail_to_body= log10(tail_length_mm/body_length_mm)) %>%
  distinct(ratio_of_hindlimb_to_body, ratio_of_tail_to_body, .keep_all = TRUE)
...
```{r}
#anolis %>%
 # ggplot(aes(x= ratio_of_hindlimb_to_body, y= habitat))+ 
  #geom_boxplot(mapping= aes(fill=habitat))+
#  labs( title = "How the Ratio of Hindlimb length and Body in mm compares among different habitats", x= "Hindlimb to Body Ratio (mm)", y="Habitat" )
...
#This Graph Suggest that the larger your body is the larger your tail will be, in relation about the lizards in the Grass bush habitat, what I notice about them is that that even though their body size is relatively small, their tail size is 3 times the amount their body size is. 
```
\
\
\
...
```{r}
anolis %>%
  filter(toepad_lamellae_count >=0 ) %>%
...
ggplot(data= filtered, aes(x=toepad_lamellae_count, y=island)) + 
  geom_boxplot(mapping=aes(fill=island)) + 
  labs(title = "Range of Toepad Lamella by island for trunk-crown species only", x= "Count of Toepad Lamellae", y= "Island")
