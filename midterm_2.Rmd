...
```{r}
register_stadiamaps("09502871-fd23-4b2b-9069-9bacd864d287", write = FALSE)
```
...
ggplot(data=polar_bears,
       mapping= aes(x=sex)) + geom_bar(mapping=aes(fill=popn)) + scale_x_discrete(na.translate= FALSE) + 
  labs(title= "Samping Between Sex and Population", x="Sex", y= "Amount per Sex and Population")
```
...
```{r}
polar_bears %>%
  summarize(mean_urea=mean(urea_nitrogen, na.rm=T),
            median_urea=median(urea_nitrogen, na.rm= T),
            min_urea= min(urea_nitrogen, na.rm= T),
...
```{r}
polar_bears %>%
  summarize(mean_creat= mean(creatinine, na.rm=T),
            median_creat= median(creatinine, na.rm = T),
            min_creat= min(creatinine, na.rm= T),
            max_creat= max(creatinine, na.rm= T))
...
ggmap(map1) +
  geom_point(data= polar_bears, aes(long,lat), size = 1, popn="color", alpha = 0.8) + 
